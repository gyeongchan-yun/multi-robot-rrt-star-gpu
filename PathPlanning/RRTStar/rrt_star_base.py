...
def draw_graph_multi_robot(rrt_star_robots, path_list, space_size):
    plt.clf()
    ...
    for robot, path, color in zip(rrt_star_robots, path_list, color_list3):
        ...
        plt.axis("equal")
        axis = [space_size[0], space_size[1], space_size[0], space_size[1]]
        plt.axis(axis)
        plt.grid(True)
        plt.pause(0.01)
    plt.show()
...
def main():
    ...
    # Draw final path
    if show_animation:
        draw_graph_multi_robot(rrt_star_robots, path_list, space_size)
